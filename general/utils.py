...
# #             if [f(**dict(zip(i, h))) for h in t] == [1, 0, 0]:
# #                 res.append(i)
# # print(len(set(res)))
...
# file = open(r"D:\informaticsclass\egeinfo\26\26_1__3whs4 (3).txt")
# n = int(file.readline())
# left = []
# right = []
# for i in range(n):
#     l, r = list(map(int, file.readline().split()))
#     if l < r: left.append([l, i + 1])
#     else: right.append([r, i + 1])

# left.sort()
# right.sort(reverse=True)
# line = left + right
# # увидели что числовое значение детали в левом списке больше
# # тогда она является последней размещенной
# print(left[-1], right[1])
# print(len(left), left[-1][-1])
